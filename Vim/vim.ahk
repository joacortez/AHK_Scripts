...
GroupAdd, WinTest, ahk_exe PowerToys.PowerLauncher.exe
GroupAdd, WinTest, ahk_exe HxOutlook.exe
GroupAdd, WinTest, - Outlook
...
check_replace_mode()
{
    return (WinActive("ahk_group WinTest") and ( currMode & replaceModeCode))
}
...
change_to_replace_mode()
{
    global
    currMode := replaceModeCode
...
substitute() {
    ...
    InputBox, regex, Regex, Enter regex to replace, , 300, 150
    ...
    ; parse "/" from regex
    regex := RegExReplace(regex, "(?<!\\)\/", "cutoff___")
    regex := RegExReplace(regex, "\/", "\\/")

    parsed := []
    
    parsed := StrSplit(regex, "cutoff___")

    quesito := RegExReplace(source, parsed[1], parsed[2])
    Clipboard := quesito
...
; STUB block duplication for Notion
...
; STUB fix notion ai math formatting
+M::
    backup := Clipboard
    Clipboard := ""
    Send ^{c}
...
; NOTE stub for notion navigation
!j:: 
    Send {Down}
return
